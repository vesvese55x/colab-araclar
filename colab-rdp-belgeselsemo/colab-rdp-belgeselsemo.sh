...
wget -q -O /etc/xrdp/startwm.sh https://raw.githubusercontent.com/vesvese55x/colab-araclar/refs/heads/main/startwm.sh && chmod +x /etc/xrdp/startwm.sh
sudo sed -i 's/^jpeg_quality=.*/jpeg_quality=25/' /etc/xrdp/xrdp.ini
sudo sed -i 's/^max_bpp=.*/max_bpp=16/' /etc/xrdp/xrdp.ini
sudo sed -i 's/^use_compression=.*/use_compression=yes/' /etc/xrdp/xrdp.ini
sudo sed -i 's/^compression_level=.*/compression_level=9/' /etc/xrdp/xrdp.ini
sudo sed -i 's/^crypt_level=.*/crypt_level=low/' /etc/xrdp/xrdp.ini
sudo sed -i 's/^username=.*/username=root/' /etc/xrdp/xrdp.ini
sudo sed -i 's/^password=.*/password=belgeselsemo/' /etc/xrdp/xrdp.ini
sudo systemctl restart xrdp > /dev/null 2>&1

# Cloudflared Kur ve RDP Tüneli Oluştur
...
cloudflared tunnel --url rdp://localhost:3389 > cloudflared.log 2>&1 &
sleep 5
...
if [ -z "$link" ]; then
  ...
  echo -e "\e[1;35mKomutu Terminalinize yazın ve Enter'a tıklayınız:\e[1;31m  cloudflared access rdp --hostname $link --url localhost:3390\e[0m"
  echo -e "\e[1;35mRDP Programınızla şu bağlantıdan bağlanabilirsiniz:\e[1;31m localhost:3390\e[0m"
  ...
  echo -e "\n\e[1;30mBu içerik \e[1;34mhttps://github.com/foxytouxxx/xrdp-and-ngrok-installer\e[1;30m Github Reposu baz alınarak, \e[1;34mCHATGPT\e[1;30m ve \e[1;34mDEEPSEEK\e[1;30m sayesinde \e[1;34mhttps://belgeselsemo.com.tr\e[1;30m sitesi tarafından oluşturulmuştur.\e[0m"
  echo -e "\n\n\e[1;36m============================ ENGLISH INSTRUCTIONS ============================\e[0m"
  echo -e "\e[1;35mType the command into your terminal: Press Enter:\e[1;31m cloudflared access rdp --hostname $link --url localhost:3390\e[0m"
  echo -e "\e[1;35mRDP You can connect using your program via the following link:\e[1;31m localhost:3390\e[0m"
  ...
  echo -e "\n\e[1;30mThis content is based on the \e[1;34mhttps://github.com/foxytouxxx/xrdp-and-ngrok-installer\e[1;30m Github Repository, created with the help of \e[1;34mCHATGPT\e[1;30m and \e[1;34mDEEPSEEK\e[1;30m by \e[1;34mhttps://belgeselsemo.com.tr\e[1;30m.\e[0m"
